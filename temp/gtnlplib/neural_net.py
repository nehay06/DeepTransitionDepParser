...
class BiLSTMWordEmbeddingLookup(nn.Module):
    ...
    def forward(self, sentence):
        ...
        wordEmbedding = wordEmbedding.expand(1, wordEmbedding.size()[0], wordEmbedding.size()[1])
...
class LSTMCombinerNetwork(nn.Module):
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        """
        # STUDENT
        inputVector = utils.concat_and_flatten([head_embed, modifier_embed])
        inputVector = inputVector.expand(1, 1, inputVector.size()[1])
        output, hidden = self.lstm(inputVector, self.hidden)
        self.hidden = hidden
...
class ActionChooserNetwork(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.output_dim = Actions.NUM_ACTIONS
        self.hidden = input_dim

        self.model = nn.Sequential(
            nn.Linear(self.input_dim,self.hidden),
            nn.ReLU(),
            nn.Linear(self.hidden, Actions.NUM_ACTIONS),
            nn.LogSoftmax()
        )
