...
class ParserState:
    ...
    def done_parsing(self):
        ...
        """
        if self.input_buffer[self.curr_input_buff_idx].headword == consts.END_OF_INPUT_TOK and len(self.stack) == 1:
            return True
        else:
            return False
    ...
    def _reduce(self, action):
        ...
        if action == Actions.REDUCE_L:
            ...
            embedding = self.combiner(head.embedding, modifier.embedding)
            stackEntry = StackEntry(head.headword, head.headword_pos, embedding)
...
class TransitionParser(nn.Module):
    ...
    def forward(self, sentence, actions=None):
        ...
        word_embedding = self.word_embedding_component(sentence)
        ...
        while (parser_state.done_parsing() == False):
            ...
            log_prob = self.action_chooser(feats)
            outputs.append(log_prob)

            pred_action = utils.argmax(log_prob)

            if have_gold_actions:
                pred_action = action_queue.popleft()

            if pred_action == Actions.REDUCE_R and len(parser_state.stack) >= 2:
                dep_graph.add(parser_state.reduce_right())
            elif pred_action == Actions.REDUCE_L and len(parser_state.stack) >= 2:
                dep_graph.add(parser_state.reduce_left())
            else:
                if parser_state.input_buffer[parser_state.curr_input_buff_idx].headword == END_OF_INPUT_TOK and len(parser_state.stack) > 1:
                    pred_action = Actions.REDUCE_R
                    dep_graph.add(parser_state.reduce_right())
                else:
                    pred_action = Actions.SHIFT
                    parser_state.shift()

            actions_done.append(pred_action)

        if len(parser_state.stack)==1:
            dep_graph.add(
                DepGraphEdge((ROOT_TOK, -1), (parser_state.stack[-1].headword, parser_state.stack[-1].headword_pos)))
